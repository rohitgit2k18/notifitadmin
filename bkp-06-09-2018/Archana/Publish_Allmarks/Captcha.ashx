<%@ WebHandler Language="C#" Class="AbleCommerce.Captcha" %>
using System;
...
namespace AbleCommerce
{
    ...
    public class Captcha : IHttpHandler
    {
        ...
        /// </summary>
        private const int _MaxImageHeight = 600;
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            int width = AlwaysConvert.ToInt(queryString["w"]);
            if (width < 1) width = 300;
            if (width > _MaxImageWidth) width = _MaxImageWidth;
            int height = AlwaysConvert.ToInt(queryString["h"]);
            if (height < 1) height = 80;
            if (height > _MaxImageHeight) height = _MaxImageHeight;
