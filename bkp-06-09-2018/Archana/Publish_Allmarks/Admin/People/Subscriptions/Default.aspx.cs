...
public partial class _Default : CommerceBuilder.UI.AbleCommerceAdminPage
{
    ...
    private bool ShowGroup(IList<SubscriptionPlan> plans)
    {
    ...
    protected void SubscriptionGrid_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if ((e.Row != null) && (e.Row.RowType == DataControlRowType.DataRow) && (e.Row.DataItem != null))
        {
            Subscription subscription = (Subscription)e.Row.DataItem;
            PlaceHolder phProcessingStatus = AbleCommerce.Code.PageHelper.RecursiveFindControl(e.Row, "phProcessingStatus") as PlaceHolder;
            if (phProcessingStatus != null)
            {
                Image pStatus = new Image();
                pStatus.SkinID = GetProcessingStatusSkinID(subscription);
                pStatus.AlternateText = subscription.ProcessingStatus.ToString();
                pStatus.ToolTip = subscription.ProcessingStatus == ProcessingStatus.OK ? "OK" : subscription.ProcessingStatusMessage;
                phProcessingStatus.Controls.Add(pStatus);
            }
        }
    }
    ...
    protected string GetProcessingStatusSkinID(Subscription subscription)
    {
        switch (subscription.ProcessingStatus)
        {
    ...
    protected void DeactivateButton_Click(object sender, EventArgs e)
    {
        ...
        foreach (DataKey item in selectedItems)
        {
            int subscriptionId = AlwaysConvert.ToInt(item.Value);
            Subscription subscription = SubscriptionDataSource.Load(subscriptionId);
            if (subscription != null) subscription.Deactivate();
        }
    ...
    {
        DeactivateButton.Visible = ActiveOnly.SelectedValue != "False";
    }
