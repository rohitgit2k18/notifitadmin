namespace AbleCommerce.Mobile.UserControls.Checkout.PaymentForms
{
    ...
    public partial class CreditCardPaymentForm : System.Web.UI.UserControl
    {
        ...
        protected void Page_Init(object sender, EventArgs e)
        {
            ...
            // SHOW AMEX HELP PANEL
            AmexPanel.Visible = _methods.Where(method => method.PaymentInstrumentType == PaymentInstrumentType.AmericanExpress).Count() > 0;
            ...
            // javascript to close the help popup when clicked on background
            string jsClosePopup = "window.onload = function() { var popup = $find('PnlHelpPopup'); if(popup) popup._backgroundElement.onclick = function() { $find('PnlHelpPopup').hide(); }};";
            ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "jsClosePopup", jsClosePopup, true);
        }
        ...
        {
            if (Page.IsPostBack && trAmount.Visible)
            {
