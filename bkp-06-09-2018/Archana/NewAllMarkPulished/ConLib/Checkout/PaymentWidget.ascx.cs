namespace AbleCommerce.ConLib.Checkout
{
    ...
    [Description("The main payment widget on payment page")]
    public partial class PaymentWidget : System.Web.UI.UserControl
    {
        ...
        public event CouponAppliedEventHandler CouponApplied;
        ...
        {
            // UPDATE THE PAYMENT METHOD LIST IF NEEDED
            if (this.Basket != null)
            {
                // payment amount is basket total
                decimal paymentAmount = this.Basket.Items.TotalPrice();
                paymentAmount = Math.Abs(paymentAmount);
                if (paymentAmount <= 0 && !(HasRecurringSubscriptions() && AbleContext.Current.Store.Settings.ROCreateNewOrdersEnabled))
                {
                    ZeroValuePaymentForm.Visible = true;
                ...
                else if(ZeroValuePaymentForm.Visible)
                {
        ...
        private void HandleCheckingOut()
        {
            CheckPaymentForm.CheckingOut += new CheckingOutEventHandler(CheckingOutHandler);
            CreditCardPaymentForm.CheckingOut += new CheckingOutEventHandler(CheckingOutHandler);
            GiftCertificatePaymentForm.CheckingOut += new CheckingOutEventHandler(CheckingOutHandler);
            MailPaymentForm.CheckingOut += new CheckingOutEventHandler(CheckingOutHandler);
            PayPalPaymentForm.CheckingOut += new CheckingOutEventHandler(CheckingOutHandler);
            PhoneCallPaymentForm.CheckingOut += new CheckingOutEventHandler(CheckingOutHandler);
            PurchaseOrderPaymentForm.CheckingOut += new CheckingOutEventHandler(CheckingOutHandler);
            ZeroValuePaymentForm.CheckingOut += new CheckingOutEventHandler(CheckingOutHandler);
        }
        ...
        private void HandlePostOrderPayment()
        {
            ...
            ZeroValuePaymentForm.CheckingOut += new CheckingOutEventHandler(PostCheckoutPayment);
        }
        ...
        private void HandleCheckoutResponse()
        {
            ...
            ZeroValuePaymentForm.CheckedOut += new CheckedOutEventHandler(HandleCheckoutResponse);
        }
        ...
        private void CouponAppliedHandler(object sender, EventArgs e)
        {
            if (CouponApplied != null) CouponApplied(sender, e);
        }
        ...
        private void PostCheckoutPayment(object sender, CheckingOutEventArgs e)
        {
            ...
            if (e.Payment.Amount > 0 && e.Payment.Amount <= Order.GetCustomerBalance())
            {
                ...
                if (result) Response.Redirect(AbleCommerce.Code.NavigationHelper.GetStoreUrl(this.Page, "Members/MyOrder.aspx?OrderNumber=" + this.Order.OrderNumber));
            }
